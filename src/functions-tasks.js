...
function getArgumentsCount(funcs) {
  return funcs.map((singleFn) => singleFn.length);
}
...
function getPolynom(...args) {
  ...
  return function polynom(x) {
    return args.reduce((result, coeff, idx) => result + coeff * x ** idx);
  };
}
...
function retry(func, attempts) {
  ...
  return function retryAttempts(...args) {
    try {
      return func(...args);
    } catch (error) {
      if (currAttempt < attempts) {
        currAttempt += 1;
        return retryAttempts.call(this, ...args);
      }
...
function logger(func, logFunc) {
  ...
  const startStr = 'starts';
  const endStr = 'ends';

  const getArgumentsStr = (argsArr) =>
    argsArr.map((elem) => JSON.stringify(elem)).join(',');
  const getFunctionLogStr = (argsArr) => {
    const argsStr = getArgumentsStr(argsArr);
    const funcPartStr = `${fnName}(${argsStr})`;
    return funcPartStr;
  };
  const getLogStr = (funcPart, suffix) => `${funcPart} ${suffix}`;

  return function logWrapper(...args) {
    const funcPartStr = getFunctionLogStr(args);
    const logStartStr = getLogStr(funcPartStr, startStr);
    const logEndStr = getLogStr(funcPartStr, endStr);
    logFunc.call(this, logStartStr);
    const result = func(...args);
    logFunc.call(this, logEndStr);
    return result;
  };
}
 ...
 *   partialUsingArguments(fn, 'a','b','c','d')() => 'abcd'
 */
function partialUsingArguments(callbackFn, ...initArgs) {
  return function partial(...nextArgs) {
    const combinedArgs = initArgs.concat(nextArgs);
    if (combinedArgs.length >= callbackFn.length) {
      return callbackFn.apply(this, combinedArgs);
    }
    return partialUsingArguments.call(this, callbackFn, ...combinedArgs);
  };
}
...
function getIdGeneratorFunction(startFrom) {
  let counter = ['number', 'bigint'].includes(typeof startFrom) ? startFrom : 0;

  return function getId() {
    const currVal = counter;
    counter += 1;
    return currVal;
  };
}
